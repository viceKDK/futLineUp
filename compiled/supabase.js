(function initSupabase() {
  ...
  window.fcCloud = {
    async uploadLocal({
      ...
      if (bytes > window.FC_BACKUP_MAX_BYTES) throw new Error("Tus datos superan el límite de sincronización de 5 MB. Exportá un backup y reducí las fotos.");
      const {
        data: remote,
        error: readError
      } = await client.from("user_backups").select("updated_at").eq("user_id", session.user.id).maybeSingle();
      ...
      if (!force && remote?.updated_at && remote.updated_at !== lastSync) {
        const conflict = new Error("Hay una versión más nueva en la nube. Recuperala o confirmá que querés reemplazarla.");
    ...
    async downloadToLocal() {
      ...
      const {
        data,
        error
      } = await client.from("user_backups").select("payload,updated_at").eq("user_id", session.user.id).maybeSingle();
      if (error) throw error;
      if (!data?.payload) throw new Error("La cuenta todavía no tiene un backup");
      window.validateFutbolClubData(data.payload);
      window.downloadJSON(window.exportFutbolClubData(), `futbolclub-antes-de-sincronizar-${new Date().toISOString().slice(0, 10)}.json`);
