...
export function measureSource(file, source) {
  ...
  return {
    ...
    decisions,
    functions,
    decisionDensity: Number((decisions / Math.max(lines, 1)).toFixed(3)),
  };
}
export async function collectSourceQuality(root = process.cwd()) {
  const base = resolve(root),
    files = (await listFiles(resolve(root, "src"))).filter((file) =>
      /\.(?:js|jsx)$/.test(file),
    );
  return Promise.all(
...
export function migrationDebt(metrics, targetLines = 500) {
  return metrics
    .filter((metric) => metric.lines > targetLines)
    .map((metric) => ({ ...metric, target: targetLines }));
}
if (
  process.argv[1] &&
  ...
  const metrics = await collectSourceQuality(),
    ...
    debt = migrationDebt(metrics);
  await mkdir(".dist/quality", { recursive: true });
  await writeFile(
    ".dist/quality/source-metrics.json",
    JSON.stringify({ targetLines: 500, metrics, debt, violations }, null, 2),
  );
  console.table(metrics.sort((a, b) => b.lines - a.lines).slice(0, 20));
  if (debt.length)
    console.warn(
      `${debt.length} archivos legacy siguen sobre el objetivo de 500 líneas.`,
    );
