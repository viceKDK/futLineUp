...
test("captura screenshots de cada seccion", async ({ page }, testInfo) => {
  ...
  const dir =
    testInfo.project.name === "chromium"
      ? "screenshots"
      : `screenshots/${testInfo.project.name}`;
  ...
  if (await fixtureTab.count()) {
    ...
    await page.getByRole("tab", { name: "Copa" }).click();
    await page.waitForTimeout(200);
    const sizeBtn = page.locator(".cup-setup .seg button", {
      hasText: "4 equipos",
    });
    ...
    // La versión actual puede mostrar el setup de liga sin pestañas.
    await shootFull(page, `${dir}/10b-league-fixture.png`);
    await shootFull(page, `${dir}/10c-league-cup.png`);
  }
  ...
  await page.goto("/futbolClub.html#auth", { waitUntil: "commit" });
