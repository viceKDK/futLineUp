...
export async function seedDemoData(page) {
  await page.evaluate(() => {
    const roster = window.db
      .load("roster", window.DEFAULT_ROSTER)
      .slice(0, 9)
      .map((p, i) => ({
        ...p,
        attrs: {
    ...
    for (let i = 10; i >= 1; i--) {
      ...
      sessions.push({
        id,
        title: i % 3 === 0 ? "Partido amistoso" : "Entrenamiento",
        date: d.toISOString().slice(0, 10),
      });
      attendance[id] = roster
        .filter((_, idx) => (idx + i) % 4 !== 0)
        .map((p) => p.id);
    }
    ...
    roster.forEach((p, pi) => {
      for (let i = 0; i < 5; i++) {
        ...
        evaluations.push({
          ...
          date: d.toISOString().slice(0, 10),
          rating: 5 + ((i + pi) % 5),
          good: "Buen desempeño en el mediocampo",
          improve: "Mejorar definición de espaldas al arco",
          goal: "Sumar minutos de rodaje",
    ...
    window.db.save("objectives", [
      {
        ...
        text: "Sumar 2 goles en julio",
        done: true,
      },
      {
        id: "ob2",
        playerId: roster[0].id,
        text: "Asistencia sobre 80%",
        done: true,
      },
      {
        id: "ob3",
        playerId: roster[0].id,
        text: "Mejorar definición pierna izquierda",
        done: false,
      },
    ]);
    ...
    const competition = {
      id: "c1",
      name: "Liga de los sábados",
      season: "2026 · Apertura",
      teams,
      fixtures,
    };
    // El módulo de liga actual usa competencias separadas; conservar también
    // la clave legacy permite que el fixture siga siendo compatible.
    window.db.save("competitions", [competition]);
    window.db.save("activeCompetitionId", "c1");
    window.db.save("league", competition);
  });
}
...
export async function openSeededApp(page) {
  ...
  // Los datos ya están persistidos y la navegación interna los leerá al montar
  // cada sección; no hace falta reiniciar la página (y evita esperar recursos
  // externos que mantienen abierto el evento de navegación).
