...
function SharePage() {
  ...
  const [shareKitMode, setShareKitMode] = React.useState(
    draft.activeKit === "alt" ? "alt" : "main",
  );
  ...
  try {
    ...
    encodedSnapshot = window.encodeLineupSnapshot({
      ...sharePayload,
      roster: sharePayload.roster.map((player) => ({ ...player, photo: null })),
    });
  }
  ...
  const downloadICS = () => {
    try {
      ...
      const dt =
        match.date.replace(/-/g, "") + "T" + match.time.replace(":", "") + "00";
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">
            Descargá como imagen, PDF o evento de calendario · link directo o
            deep-link a redes.
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button className="btn" onClick={downloadPNG}>
            <Icon name="download" size={13} /> Descargar PNG
          </button>
          <button className="btn primary" onClick={nativeShare}>
            <Icon name="send" size={13} /> Compartir
          </button>
        </div>
      </div>
      {shareWasCompressed && (
        <div className="share-size-note">
          Para que el enlace funcione bien en móviles y mensajería, se
          compartirá sin las fotos de jugadores.
        </div>
      )}
            ...
            {styleTab === "card" && (
              <div className="share-card" ref={cardRef}>
                <div className="share-card-head">
                  <div>
                    {include.venue && (
                      <div className="share-kicker">{kicker}</div>
                    )}
                    <div className="share-title">
                      {(draft.name || "MI EQUIPO").toUpperCase()}
                    </div>
                  </div>
                  {include.kit && (
                    <Kit
                ...
                <div className="share-card-foot">
                  ...
                  {include.venue ? (
                    ...
                    <div className="share-meta-item">
                      <span>Jugadores</span>
                      <strong>
                        {players.filter(Boolean).length}/{size}
                      </strong>
                    </div>
                  )}
                </div>
                ...
                {include.watermark && (
                  <div className="share-watermark">futbolClub.app</div>
                )}
              </div>
            )}
                ...
                <div className="share-list-grid">
                  {players.filter(Boolean).map((p) => (
                    <div key={p.id} className="share-list-item">
                      ...
                      <div>
                        {include.names && (
                          <div className="share-list-name">{p.name}</div>
                        )}
            ...
            {styleTab === "stories" && (
              <div className="share-card stories" ref={cardRef}>
                ...
                >
                  ...
                  <div className="share-title" style={{ fontSize: 48 }}>
                    {(draft.name || "MI EQUIPO").toUpperCase()}
                  </div>
        ...
        <div className="share-side">
            ...
            <div className="match-fields">
              <label>
                ...
                <input
                  ...
                  value={match.date}
                  onChange={(e) =>
                    setMatch((m) => ({ ...m, date: e.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  value={match.time}
                  onChange={(e) =>
                    setMatch((m) => ({ ...m, time: e.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  value={match.venue}
                  onChange={(e) =>
                    setMatch((m) => ({ ...m, venue: e.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setMatch((m) => ({ ...m, opponent: e.target.value }))
                  }
                />
              </label>
            </div>
          ...
          <div className="panel">
            ...
            <div className="share-socials">
              <button className="social wa" onClick={openWA}>
                WhatsApp
              </button>
              <button className="social ig" onClick={openIG}>
                Instagram
              </button>
              <button className="social tw" onClick={openTW}>
                X / Twitter
              </button>
              <button className="social tg" onClick={openTG}>
                Telegram
              </button>
            </div>
          </div>
          ...
          <div className="panel">
            ...
            <label className="toggle-row">
              <input
                ...
                onChange={() => tog("names")}
              />{" "}
              <span>Nombres de jugadores</span>
            </label>
            <label className="toggle-row">
              <input
                type="checkbox"
            ...
            <label className="toggle-row">
              <input
                ...
                onChange={() => tog("stats")}
              />{" "}
              <span>Estadísticas último partido</span>
            </label>
            <label className="toggle-row">
              <input
                type="checkbox"
          ...
          <div className="panel">
            ...
            <div className="export-grid">
              <button className="export-opt" onClick={downloadPNG}>
                PNG
                <br />
                <span>1080×1350</span>
              </button>
              <button className="export-opt" onClick={downloadPDF}>
                PDF
                <br />
                <span>A4</span>
              </button>
              <button className="export-opt" onClick={downloadICS}>
                .ics
                <br />
                <span>Calendario</span>
              </button>
              <button className="export-opt" onClick={copyLink}>
                Link
                <br />
                <span>Vista web</span>
              </button>
...
window.mountPage("page-share", <SharePage />);
