...
export function createSnapshotService({ codec, supportsMode }) {
  function validate(snapshot) {
    if (!snapshot || snapshot.v !== 1 || !snapshot.draft || typeof snapshot.draft !== "object" || Array.isArray(snapshot.draft))
    ...
    if (Object.hasOwn(snapshot, "roster") && (!Array.isArray(snapshot.roster) || snapshot.roster.length > 100))
  ...
  return Object.freeze({
    encode(snapshot) {
      ...
      if (encoded.length > SHARE_MAX_CHARS)
        throw new Error("La alineación es demasiado grande para compartirla como enlace");
      return encoded;
    },
    decode(encoded) {
