export function createAutomaticBackupService({ repository, exportData, validate, importData, maxBytes, clock = () => new Date(), idFactory, minIntervalMs = 6 * 60 * 60 * 1000, maxBackups = 7, onEvent = () => {}, logger = () => {} }) {
  ...
  async function prune() { const backups = await list(); for (const backup of backups.slice(maxBackups)) await repository.remove(backup.id); }
  function create(reason = "automatic", { force = false } = {}) {
    ...
    creating = (async () => {
      ...
      const exported = exportData(); if (!Object.keys(exported.data || {}).length) return null;
      const payload = validate(exported), bytes = bytesOf(payload); if (bytes > maxBytes) throw new Error("El backup automático supera el límite permitido");
    ...
    })().catch((error) => { logger("error", "Falló el backup automático", { error, reason }); throw error; }).finally(() => { creating = null; });
    return creating;
  }
  async function restore(id) {
    const target = (await list()).find((backup) => backup.id === id); if (!target) throw new Error("No se encontró el backup automático");
...
export function createBackupScheduler({ backupService, delayMs = 10000, setTimer, clearTimer, logger = () => {} }) {
  if (typeof setTimer !== "function" || typeof clearTimer !== "function") throw new TypeError("El scheduler requiere puertos de temporizador");
  let timer = null;
