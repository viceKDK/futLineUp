// Keep fixture/cup selectors scoped to the active competition participants.
(function () {
  ...
  function participantNames() {
    ...
    if (explicit.length) explicit.forEach(add);
    else {
  ...
  function syncTeamPicker() {
    ...
    if (
      ...
      !current.every((value, index) => value === names[index])
    ) {
      scopedList.replaceChildren(
        ...names.map((name) => {
    ...
    root
      .querySelectorAll(
        `input[list="league-team-names"], input[list="${scopedListId}"]`,
      )
      .forEach((input) => input.setAttribute("list", scopedListId));
  }

  function fieldValue(label) {
    const wanted = normalize(label);
    const field = [...root.querySelectorAll("label.field")].find(
      (item) => normalize(item.querySelector("span")?.textContent) === wanted,
    );
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      if (/Generar cuadro/i.test(label)) {
        const entered = [...root.querySelectorAll(".cup-setup-grid input")]
  ...
  window.addEventListener("fc:data-changed", (event) => {
    if (["competitions", "activeCompetitionId", "teams"].includes(event.detail?.key))
      syncTeamPicker();
  });
  syncTeamPicker();
})();

// ---- League fixture generation + CSV export ----
const leagueFixtureEl = (tag, props, ...children) =>
...
function generateLeagueRoundRobin(
  teamValues,
  { doubleRound = false, startDate = leagueLocalDate(), daysBetween = 7 } = {},
) {
...
function leagueFixtureCsv(competition) {
  ...
  (competition?.fixtures || []).forEach((fixture) => {
    rows.push(
      [
        fixture.round || "",
        fixture.leg === 2 ? "Vuelta" : fixture.leg === 1 ? "Ida" : "",
        fixture.date || "",
        fixture.home || "",
        fixture.away || "",
        fixture.played ? fixture.homeScore : "",
        fixture.played ? fixture.awayScore : "",
      ]
        .map(leagueCsvEscape)
        .join(","),
    );
...
function LeagueFixtureGenerator({ competition, onSave, onCancel }) {
  const teams = inferLeagueTeams(competition);
  ...
  const roundCount = preview.reduce(
    (max, fixture) => Math.max(max, Number(fixture.round) || 0),
    0,
  );
  ...
  const submit = () => {
    ...
    if (
      mergeMode === "replace" &&
      existing.some((fixture) => fixture.played) &&
      !confirm("Hay resultados cargados. ¿Reemplazar igualmente todo el fixture?")
    ) {
      return;
    }
    onSave({
  ...
  return leagueFixtureEl(
    ...
    leagueFixtureEl(
      ...
      leagueFixtureEl(
        "div",
        null,
        leagueFixtureEl("div", { className: "page-kicker" }, "Calendario automático"),
        leagueFixtureEl("h1", null, "Generar fixture"),
        leagueFixtureEl(
          "p",
          null,
          `Armá automáticamente el calendario de ${teams.length} equipos. Podés usar una rueda de ida o ida y vuelta.`,
        ),
      ),
      leagueFixtureEl(
        "button",
        { className: "btn ghost", type: "button", onClick: onCancel },
        "Cancelar",
      ),
    ),
    leagueFixtureEl(
      ...
      leagueFixtureEl(
        ...
        leagueFixtureEl(
          ...
          "1",
          leagueFixtureEl("span", null, "Formato"),
        ),
        leagueFixtureEl(
          "div",
          { className: "seg", role: "group", "aria-label": "Formato del fixture" },
          leagueFixtureEl(
            "button",
            {
              type: "button",
        ...
        leagueFixtureEl(
          ...
          "2",
          leagueFixtureEl("span", null, "Fechas"),
        ),
        leagueFixtureEl(
          "div",
          ...
          leagueFixtureEl(
            "label",
            { className: "field" },
            leagueFixtureEl("span", null, "Primera fecha"),
            leagueFixtureEl("input", {
              type: "date",
              value: startDate,
          ...
          leagueFixtureEl(
            ...
            leagueFixtureEl("span", null, "Días entre fechas"),
            leagueFixtureEl("input", {
              type: "number",
              min: "1",
              max: "365",
        ...
        existing.length > 0 &&
          leagueFixtureEl(
            ...
            leagueFixtureEl(
              ...
              { className: "league-setup-step second" },
              "3",
              leagueFixtureEl("span", null, "Qué hacer con el fixture actual"),
            ),
            leagueFixtureEl(
              "div",
              { className: "seg", role: "group", "aria-label": "Destino del fixture generado" },
              leagueFixtureEl(
                "button",
                {
                  type: "button",
      ...
      leagueFixtureEl(
        ...
        leagueFixtureEl(
          ...
          { className: "league-setup-step" },
          "PREVIEW",
          leagueFixtureEl("span", null, "Calendario resultante"),
        ),
        leagueFixtureEl("h2", null, `${preview.length} partidos`),
        leagueFixtureEl(
          "p",
          { className: "league-import-copy" },
          `${roundCount} fechas · ${doubleRound ? "ida y vuelta" : "una rueda"} · cada ${Math.max(1, Number(daysBetween) || 7)} días.`,
        ),
        leagueFixtureEl(
          "div",
          { className: "league-import-result" },
          leagueFixtureEl("strong", null, teams.join(" · ")),
          preview.length > 0 &&
            leagueFixtureEl(
              "span",
              null,
              `${preview[0].date || "sin fecha"} → ${preview.at(-1)?.date || "sin fecha"}`,
            ),
        ),
        leagueFixtureEl(
          "div",
          { className: "league-csv-help" },
          leagueFixtureEl("strong", null, "Después podés editarlo"),
          leagueFixtureEl("span", null, "El fixture generado queda en la pestaña Fixture como cualquier partido normal."),
          leagueFixtureEl("span", null, "Podés cargar resultados, eliminar partidos y exportarlo a CSV."),
          teams.length % 2 === 1 &&
            leagueFixtureEl("span", null, "Hay un número impar de equipos: en cada fecha uno queda libre."),
        ),
      ),
    ),
    error &&
      leagueFixtureEl(
        "div",
        { className: "league-setup-error", role: "alert" },
        error,
      ),
    leagueFixtureEl(
      "div",
      ...
      leagueFixtureEl(
        ...
        leagueFixtureEl("strong", null, preview.length),
        leagueFixtureEl("span", null, "partidos"),
        leagueFixtureEl("i", null, "·"),
        leagueFixtureEl("strong", null, roundCount),
        leagueFixtureEl("span", null, "fechas"),
      ),
      leagueFixtureEl(
        "button",
...
LeagueWorkspace = function LeagueWorkspaceWithFixtureTools() {
  ...
  const [setupMode, setSetupMode] = React.useState(() =>
    competitions.length ? null : "create",
  );
  ...
  return leagueFixtureEl(
    ...
    leagueFixtureEl(
      ...
      leagueFixtureEl(
        "div",
        null,
        leagueFixtureEl("strong", null, activeCompetition?.name || "Liga"),
        leagueFixtureEl(
          "span",
          null,
          `${activeTeams.length} participantes · ${activeCompetition?.fixtures?.length || 0} partidos`,
        ),
      ),
      leagueFixtureEl(
        "div",
    ...
    React.createElement(LeaguePage, {
