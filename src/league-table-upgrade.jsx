// League standings upgrade: keep every participant visible from 0 PJ / 0 PTS.
(function () {
  ...
  function getTeamNames(fixtures) {
    ...
    const explicitTeams = Array.isArray(activeCompetition?.teams)
    ...
    // Competencias nuevas tienen participantes propios. Para ligas viejas sin
    // esa lista mantenemos el comportamiento anterior como compatibilidad.
    if (explicitTeams.length) {
      explicitTeams.forEach(add);
    } else {
      const savedTeams =
        window.db?.load?.("teams", window.DEFAULT_SAVED_TEAMS || []) || [];
  ...
  function buildStandings(fixtures, teamNames) {
    const table = new Map(
      [...teamNames].map((name) => [name, emptyRow(name)]),
    );
    const row = (name) => {
  ...
  window.calculateStandings = function calculateStandingsWithZeroTeams(fixtures) {
    ...
    const previousPosition = new Map(
      previous.map((team, index) => [team.name, index + 1]),
    );
    window.__leagueStandingsMovement = Object.fromEntries(
  ...
  function decorateMovement() {
    ...
    document
      .querySelectorAll("#page-league table.standings tbody tr")
      .forEach((row) => {
        ...
        marker.title =
          value > 0
            ? `Subió ${value} posición${value === 1 ? "" : "es"}`
            : `Bajó ${Math.abs(value)} posición${Math.abs(value) === 1 ? "" : "es"}`;
      });
  }
