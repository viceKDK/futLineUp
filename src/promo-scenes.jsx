...
function PromoPitch({
  ...
  return (
    ...
    >
      ...
      {vignette && (
        <rect x="0" y="0" width="100" height="150" fill="url(#vg)" />
      )}
      <g
        fill="none"
...
function PromoDot({
  ...
  return (
    <g
      ...
      opacity={opacity}
      style={{ transition: "transform .5s cubic-bezier(.65,0,.35,1)" }}
    >
  ...
  return (
    <div
      style={{
        ...
        inset: 0,
        background:
          "radial-gradient(ellipse at 50% 50%, #131815 0%, #050605 80%)",
        display: "flex",
...
function ScenePitchReveal() {
  ...
  const exit =
    localTime > duration - 0.6
      ? clamp((localTime - (duration - 0.6)) / 0.6, 0, 1)
      : 0;
  ...
  const idx = Math.min(
    modes.length - 1,
    Math.floor((localTime - cycleStart) / modeStep),
  );
  const current = modes[Math.max(0, idx)];
  const localCycleT = Math.max(
    0,
    ((localTime - cycleStart) % modeStep) / modeStep,
  );
  const modePop =
    1 + (1 - Easing.easeOutCubic(clamp(localCycleT * 4, 0, 1))) * 0.06;
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{
            width: isSquare ? 380 : 540,
            height: isSquare ? 570 : 810,
            transform: `scale(${0.92 + 0.08 * fadeIn})`,
            opacity: fadeIn * 0.55,
          }}
        >
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Arrastrá los pibes a la cancha. Elegí formación predefinida o moveté
            libre.
          </div>
          <div
            style={{
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{
            width: isSquare ? 420 : 600,
            height: isSquare ? 630 : 900,
            position: "relative",
          }}
        >
          <PromoPitch progressDraw={1} />
...
function SceneFormationChange() {
  ...
  const exit =
    localTime > duration - 0.6
      ? clamp((localTime - (duration - 0.6)) / 0.6, 0, 1)
      : 0;
  const morphStart = 1.0,
    morphEnd = 2.8;
  const morphT = Easing.easeInOutCubic(
    clamp((localTime - morphStart) / (morphEnd - morphStart), 0, 1),
  );
  ...
  const FormationArrow = ({ fs = 90, gap = 36, arrowW = 60 }) => (
    ...
    >
      <div
        style={display({
          fontSize: fs,
          color: morphT < 0.5 ? "#cefa3e" : "rgba(245,242,232,.3)",
          letterSpacing: "3px",
        })}
      >
        4-4-2
      ...
      <svg
        width={arrowW + 20}
        height="20"
        viewBox={`0 0 ${arrowW + 20} 20`}
        style={{ flexShrink: 0 }}
      >
        <path
          d={`M 0 10 L ${morphT * arrowW} 10`}
          stroke="#cefa3e"
          strokeWidth="2"
        />
        <polygon
          points={`${morphT * arrowW},5 ${morphT * arrowW + 10},10 ${morphT * arrowW},15`}
          fill="#cefa3e"
        />
      </svg>
      <div
...
function SceneKits() {
  ...
  const enter = clamp(localTime / 0.4, 0, 1);
  const exit =
    localTime > duration - 0.5
      ? clamp((localTime - (duration - 0.5)) / 0.5, 0, 1)
      : 0;
  const kits = [
  ...
  const activeIdx = Math.min(
    kits.length - 1,
    Math.max(0, Math.floor((localTime - cycleStart) / perKit)),
  );
  const localKitT = clamp(
    (localTime - cycleStart - activeIdx * perKit) / perKit,
    0,
    1,
  );
  ...
  const kitJsx = (
    ...
    >
      <Kit
        design={kits[activeIdx].design}
        primary={kits[activeIdx].primary}
        secondary={kits[activeIdx].secondary}
        number={10}
        name="FUTBOLCLUB"
        size={kitSize}
      />
    ...
    return (
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse at center, #131815, #050605 75%)",
          opacity: enter * (1 - exit),
        }}
      >
        <div
        ...
        >
          ...
          >
            4 diseños · colores libres
            <br />
            dorsal · nombre del club
          </div>
  ...
  return (
    <div
      style={{
        position: "absolute",
        inset: 0,
        background: "radial-gradient(ellipse at center, #131815, #050605 75%)",
        opacity: enter * (1 - exit),
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
...
function SceneDraw() {
  ...
  const assigned =
    localTime > spinEnd ? clamp((localTime - spinEnd) / 0.5, 0, 1) : 0;
    ...
    >
      ...
      <div
        style={display({
          ...
          justifyContent: "center",
          transform:
            t < 1 ? `translateY(${Math.sin(localTime * 30) * 4}px)` : "none",
        })}
      >
      ...
      {assigned > 0 && (
        <div
          style={display({
            ...
            color: teamColor,
            opacity: assigned,
            transform: `translateY(${(1 - assigned) * 12}px)`,
          })}
        >
          → {teamLabel}
        </div>
    ...
    >
      {[
        ...
        <div
          ...
          style={{
            padding: "12px 20px",
            background: "rgba(20,25,22,.7)",
            border: `1px solid ${tm[1]}`,
            borderTop: `4px solid ${tm[1]}`,
            borderRadius: 8,
            minWidth: 180,
          }}
        >
  ...
  return (
    ...
    >
      ...
      >
        <Spinner
          minWidth={isSquare ? 420 : 600}
          fs={isSquare ? 130 : 200}
          pad={isSquare ? "30px 50px" : "40px 70px"}
        />
      </div>
...
function SceneRival() {
    ...
    <div style={{ position: "relative", width: w, height: h, opacity: slideE }}>
      ...
      <div
        style={{
          ...
          padding: "8px 28px",
          ...display({
            fontSize: w > 700 ? 140 : 100,
            color: "#cefa3e",
            letterSpacing: "10px",
            lineHeight: 1,
          }),
          textShadow:
            "0 0 40px rgba(206,250,62,.9), 0 0 12px rgba(0,0,0,.8), 0 4px 24px rgba(0,0,0,.6)",
          zIndex: 10,
        }}
      >
        VS
      ...
      >
        ...
        <div
          style={{
            ...
            left: 80,
            top: "50%",
            transform: `translate(${(1 - vsT) * -20}px, -50%)`,
            opacity: vsT,
            width: 360,
          }}
        >
          <Kicker color="rgba(245,242,232,.5)" mb={10}>
            06 / MODO RIVAL
          </Kicker>
          <div
            style={display({
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          paddingTop: isSquare ? 70 : 130,
          opacity: vsT,
          textAlign: "center",
        }}
      >
        <Kicker color="rgba(245,242,232,.5)" mb={8}>
          VISITANTE · 4-4-2
        </Kicker>
        <div
          style={display({
            fontSize: isSquare ? 56 : 84,
            lineHeight: 0.88,
            color: "#eab308",
            letterSpacing: "4px",
          })}
      ...
      >
        ...
        <Kicker color="rgba(245,242,232,.5)" mb={0}>
          LOCAL · 4-3-3 · 21:30 · CANCHA 3
        </Kicker>
      </div>
    </div>
  );
  ...
  return (
    <div
      style={{
        ...
        inset: 0,
        background:
          "radial-gradient(ellipse at center, #131815 0%, #050605 75%)",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
    ...
    >
      ...
      <div
        style={{
          opacity: logoIn,
          transform: `scale(${0.7 + 0.3 * Easing.easeOutBack(logoIn)})`,
          marginBottom: 36,
        }}
      >
        <div
