...
function EscudosPage() {
  ...
  const allNames = [
    ...new Set([...savedTeams.map((t) => t.name), ...customNames]),
  ].sort();
  ...
  const saveCrestEntry = (name, entry) =>
    setTeamCrests((prev) => ({ ...prev, [normalizeCrestName(name)]: entry }));
  const resetCrestEntry = (name) =>
    setTeamCrests((prev) => {
      const n = { ...prev };
  ...
  const addCustomName = () => {
    ...
    if (allNames.some((n) => n.toLowerCase() === trimmed.toLowerCase()))
      return window.__toast?.("Ese equipo ya está en la lista");
    ...
    <div>
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">
            Opcional — se usan en Liga amateur, Copa y Modo rival. Sin nada
            elegido se genera uno simple con colores e iniciales.
          </div>
        </div>
        <div />
      </div>
      ...
      <div className="card crest-add-card">
        ...
        <div className="crest-add-row">
          ...
          />
          <button className="btn primary" onClick={addCustomName}>
            <Icon name="plus" size={14} /> Agregar
          </button>
        </div>
        <p className="muted">
          Tus equipos guardados ya aparecen abajo. Agregá acá rivales que
          todavía no cargaste en ningún partido, para prepararles el escudo con
          anticipación.
        </p>
      </div>
      ...
      <div className="card">
        <div className="panel-head-row">
          <span>Equipos · {allNames.length}</span>
        </div>
        {allNames.length ? (
          <div className="crest-manager-grid">
            {allNames.map((name) => (
          ...
          <div className="empty-state">
            Todavía no hay equipos. Guardá uno en "Mis equipos" o agregá un
            rival arriba.
          </div>
        )}
      </div>
...
function CrestEditorScreen({ name, entry, onSave, onReset, onBack }) {
  ...
  const [primary, setPrimary] = React.useState(
    base.primary || window.colorFor(name || "?"),
  );
        ...
        <div className="kit-preview">
          <div className="crest-preview-stage">
            ...
            <label className="toggle-row crest-hidden-toggle">
              ...
              />{" "}
              <span>Sin escudo (no mostrar nada)</span>
            </label>
          </div>
        ...
        <div className="kit-controls">
          ...
          <div className="panel">
            ...
            <div className="swatches">
              {window.KIT_COLOR_SWATCHES.map((c) => (
                <button
                  key={c}
          ...
          <div className="panel">
            ...
            <div className="crest-photo-row">
              ...
              {photo && (
                <button className="btn sm ghost" onClick={() => setPhoto(null)}>
                  Quitar foto
                </button>
              )}
              <input
...
window.mountPage("page-crests", <EscudosPage />);
