...
function Pitch(props) {
  ...
  const positions = rawPositions.map(
    (p, i) => (freeMode && positionOverrides && positionOverrides[i]) || p,
  );
  ...
  const lineColor =
    style === "dark" ? "rgba(255,255,255,.5)" : "rgba(255,255,255,.85)";
  ...
  const startPlayerDrag = (e, idx) => {
    ...
    const move = (ev) => {
      if (!state.dragging) {
        ...
        svg
          .querySelector(`[data-slot-idx="${idx}"]`)
      ...
      const hoverEl =
        slotEl && svg.contains(slotEl) && slotEl.dataset.slotIdx != String(idx)
          ? slotEl
          : dropEl;
      if (state.hoverEl && state.hoverEl !== hoverEl)
        state.hoverEl.classList.remove("hover");
  ...
  return (
    ...
    >
      ...
      {freeMode && (
        <div className="pitch-free-hint">
          MODO LIBRE · arrastrá los círculos
        </div>
      )}
      <svg
        ref={svgRef}
...
function PlayerDot({ player, kit, interactive, showName, onRemove }) {
  ...
  const display =
    (typeof window !== "undefined" &&
      window.document?.body?.dataset?.playerStyle) ||
    "photo";
    ...
    <svg width="100%" height="100%" viewBox="0 0 100 105">
      ...
      <text
        ...
        style={{
          paintOrder: "stroke",
          stroke:
            window.contrastTextMixed(kit.primary, kit.secondary, kit.design) ===
            "#ffffff"
              ? "rgba(0,0,0,.55)"
              : "rgba(255,255,255,.65)",
          strokeWidth: 1.8,
